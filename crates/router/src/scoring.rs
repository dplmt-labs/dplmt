...
impl SortMode {
    pub fn weights(self) -> Weights {
        match self {
            SortMode::Cheapest => Weights { amount: 0.7, duration: 0.15, security: 0.1, slippage: 0.05 },
            SortMode::Fastest => Weights { amount: 0.1, duration: 0.7, security: 0.15, slippage: 0.05 },
            SortMode::Safest => Weights { amount: 0.1, duration: 0.1, security: 0.7, slippage: 0.1 },
            SortMode::Balanced => Weights { amount: 0.4, duration: 0.3, security: 0.2, slippage: 0.1 },
        }
    }
}
...
pub fn score_quotes(quotes: &[BoothQuote], mode: SortMode) -> Vec<ScoredQuote> {
    ...
    let max_slip = quotes.iter().map(|q| q.slippage_pct).fold(0.0_f64, f64::max).max(1e-4);
    let dur_spread = (max_dur - min_dur).max(1.0);

    let mut scored: Vec<ScoredQuote> = quotes
        .iter()
        .map(|q| {
            let amount_norm = if max_out > 0.0 { 1.0 - q.amount_out / max_out } else { 1.0 };
            ...
            let score = w.amount * amount_norm + w.duration * dur_norm + w.security * sec_norm + w.slippage * slip_norm;
            ScoredQuote { quote: q.clone(), score, rank: -1 }
        })
        .collect();

    let mut sorted_idx: Vec<usize> = (0..scored.len()).collect();
    sorted_idx.sort_by(|a, b| scored[*a].score.partial_cmp(&scored[*b].score).unwrap_or(std::cmp::Ordering::Equal));
...
pub fn best_by(quotes: &[BoothQuote]) -> Option<BestBy> {
    ...
    let cheapest = quotes
        .iter()
        .max_by(|a, b| a.amount_out.partial_cmp(&b.amount_out).unwrap_or(std::cmp::Ordering::Equal))?
        .booth;
    let fastest = quotes
        .iter()
    ...
    Some(BestBy { cheapest, fastest, safest, balanced })
}
